...
2. 对 framebuffer 进行加锁，在一帧完整数据写完之前，不能将其 post 到显示屏显示，若 Vsync 信号到，则仍然显示上一帧；同样，在一帧数据写完之后被 post 之前，不能写入下一帧数据。

Vsync 的加锁作用其实就能够解决 tearing 现象，因为加锁能够保证在显示帧时机要么取到的是一个完整帧，要么取不到新帧，这样在显示端任何时刻都显示的是一个完整的帧。但是在单个 framebuffer 的情况下，很难在每个显示时机都刷新到新帧，而是会连续出现好几个老帧，这就是 **jank现象**。如下图所示：

![Jank single framebuffer](images/jank_single_framebuffer.png "Jank single framebuffer")

可以看到，第一个 Vsync 信号到达的时候，显示设备试图取第2帧显示，framebuffer 已经准备好，可传送到显示设备，同时 CPU/GPU 试图生成第3帧，但是因为 framebuffer 中的帧还未被取走显示，被锁定了，只能等待下一个 Vsync 信号到来才能继续生成；第二个 Vsync 信号到达的时候，显示设备试图取第3帧，但是因为 framebuffer 已经被消费完了（单缓冲），被插入到了生成队列等待第3帧生成，相当于被锁定了，于是只能继续显示第2帧，产生了 jank 现象。

上图已经展示了生成帧的时机与显示帧的时机同步了，即都与 Vsync 信号同步了。不难推断，如果 Vsync 只有加锁作用而没有同步作用，那么对于生成帧速度 < 显示帧速度的情况，产生 jank现象的几率会更大，而有了 Vsync 同步作用后，Vsync 会给生成帧过程提速，在 Vsync 信号到达后，强制开始下一帧的生成。下图展示了没有 Vsync 同步作用的情况：

![Jank without sync](images/jank_without_sync.png "Jank without sync")

值得注意的是，上图中已经不是单缓冲的情况了，但是仍然因为没有同步作用而产生了 jank现象。由此可见，Vsync 加锁可以解决 tearing问题，而 Vsync同步和多缓冲才能解决 jank问题。

接下来看看 framebuffer **双缓冲** 的情况：
