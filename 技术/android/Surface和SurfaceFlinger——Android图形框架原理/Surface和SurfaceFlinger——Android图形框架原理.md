...
Vsync 的加锁作用其实就能够解决 tearing 现象，因为加锁能够保证在显示帧时机要么取到的是一个完整帧，要么取不到新帧，这样在显示端任何时刻都显示的是一个完整的帧，只不过有可能同一帧会连续出现几次，这样给用户的感觉就是显示的动画有卡顿，这个叫 jank 现象，如下图所示：



Vsync 可以让生成帧和显示帧的时机基本一致：对于生成帧速度 > 显示帧速度的情况，Vsync 给生成帧过程降速，保证在一个 VBI 期间在一个 framebuffer 中只产生一个新帧；对于生成帧速度 < 显示帧速度的情况，Vsync 给生成帧过程提速，在 Vsync 信号到达后，强制开始下一帧的生成。
